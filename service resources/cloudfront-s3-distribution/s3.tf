...
resource "aws_s3_bucket_object" "terraform-bucket" {
    ...
    acl    = var.acl //Error: expected acl to be one of [private public-read public-read-write authenticated-read aws-exec-read bucket-owner-read bucket-owner-full-control], got private public-read
...
resource "aws_s3_bucket_object" "aws-image" {
    ...
    acl    = var.acl //Error: expected acl to be one of [private public-read public-read-write 
}
